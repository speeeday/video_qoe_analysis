...
class Twitch_Video_Loader:
	...
	def run(self, link):
		...
		try: # lots of things can go wrong in this loop TODO - report errors 
			...
			time_search_str, bitrate_search_str = "(.+) sec.", "(.+) Kbps"
			while not stop:
				# get video progress
				# Note - this reading process can take a while, so sleeping is not necessarily advised
				t_calls = time.time()
				n_all_elements = len(self.driver.find_elements_by_css_selector(find_str + " > tr"))
				try:
					if n_all_elements == 11 and not self.is_buffering(link):
						# playing or paused
						current_optimal_res = self.driver.find_element_by_css_selector(find_str + " tr:nth-child({}) > td:nth-child(2) > p".format(1)).text
						buffer_health = self.driver.find_element_by_css_selector(find_str + " tr:nth-child({}) > td:nth-child(2) > p".format(5)).text
						buffer_health = float(re.search(time_search_str, buffer_health).group(1))
						if buffer_health == 0:
							state = "paused"
							fps = 0
						else:
							state = "playing"
							fps = int(self.driver.find_element_by_css_selector(find_str + " tr:nth-child({}) > td:nth-child(2) > p".format(3)).text)
						latency_mode = self.driver.find_element_by_css_selector(find_str + " tr:nth-child({}) > td:nth-child(2) > p".format(6)).text
						dropped_frames = int(self.driver.find_element_by_css_selector(find_str + " tr:nth-child({}) > td:nth-child(2) > p".format(4)).text)
						bitrate = self.driver.find_element_by_css_selector(find_str + " tr:nth-child({}) > td:nth-child(2) > p".format(8)).text
						bitrate = int(re.search(bitrate_search_str, bitrate).group(1))
					else:
						# buffering
						state = "rebuffer"
						current_optimal_res = self.driver.find_element_by_css_selector(find_str + " tr:nth-child({}) > td:nth-child(2) > p".format(1)).text
						buffer_health = self.driver.find_element_by_css_selector(find_str + " tr:nth-child({}) > td:nth-child(2) > p".format(4)).text
						buffer_health = float(re.search(time_search_str, buffer_health).group(1))
						latency_mode = self.driver.find_element_by_css_selector(find_str + " tr:nth-child({}) > td:nth-child(2) > p".format(5)).text
						dropped_frames = int(self.driver.find_element_by_css_selector(find_str + " tr:nth-child({}) > td:nth-child(2) > p".format(3)).text)
						fps = 0
